...
# TradingView çökerse devreye girecek BIST Lokomotif Yedek Sepeti
BIST_BACKUP_WATCHLIST = [
    "THYAO.IS", "ASELS.IS", "EREGL.IS", "TUPRS.IS", "KCHOL.IS", "GARAN.IS",
    "AKBNK.IS", "SISE.IS", "BIMAS.IS", "FROTO.IS", "SAHOL.IS", "PETKM.IS",
    "EKGYO.IS", "ISCTR.IS", "YKBNK.IS", "ENKAI.IS", "KOZAL.IS", "TOASO.IS",
    "ARCLK.IS", "PGSUS.IS", "HEKTS.IS", "KRDMD.IS", "TCELL.IS", "VESTL.IS",
    "TTKOM.IS", "SASA.IS", "ODAS.IS", "GUBRF.IS", "KOZAA.IS", "DOHOL.IS"
]
...
def get_bist_yfinance_fallback():
    ...
    try:
        ...
        print(f"BIST yedek hat hatası: {e}")
        return pd.DataFrame()

def fetch_all_data():
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 1. Hat: TradingView BIST taranıyor...")
    try:
        ...
        print(f"TradingView BIST Hatası: {e}")

    # TradingView başarısız olursa 2. Hat (Yedek yfinance)
    df_fallback = get_bist_yfinance_fallback()
